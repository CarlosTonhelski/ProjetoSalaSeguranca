...
def criar_colaborador():
    ...
    novo = Colaborador(
        ...
        rfid_tag=dados.get('rfid_tag', '').strip().upper(),
        cargo=dados.get('cargo'),
        acesso_permitido=dados.get('acesso_permitido', False),
        ativo=dados.get('ativo', True)
    )
    db.session.add(novo)
    db.session.commit()
    return jsonify({'mensagem': 'Colaborador criado'}), 201
...
def editar_colaborador(id):
    ...
    colaborador.matricula        = dados.get('matricula',        colaborador.matricula)
    colaborador.rfid_tag         = dados.get('rfid_tag',         colaborador.rfid_tag)
    colaborador.cargo            = dados.get('cargo',            colaborador.cargo)
    colaborador.acesso_permitido = dados.get('acesso_permitido', colaborador.acesso_permitido)
    colaborador.ativo            = dados.get('ativo',            colaborador.ativo)
    db.session.commit()
    return jsonify({'mensagem': 'Colaborador atualizado'})
...
def deletar_colaborador(id):
    ...
    db.session.commit()
    return jsonify({'mensagem': 'Colaborador removido'})